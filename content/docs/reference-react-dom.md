...
আপনি যদি React একটি `<script>` ট্যাগ এর মাধ্যমে লোড করে থাকেন, তাহলে নিচের top-level API গুলো `ReactDOM` গ্লোবাল ভ্যারিয়েবলে আপনার ব্যবহারের জন্য উপস্থিত থাকবে। আপনি যদি npm এর মাধ্যমে ES6 ব্যবহার করতে চান তাহলে আপনি এভাবে লিখতে পারেন `import ReactDOM from 'react-dom'`। অথবা আপনি যদি npm এর মাধ্যমে ES5 ব্যবহার করতে চান তাহলে আপনি এভাবে লিখতে পারেন `var ReactDOM = require('react-dom')`।

## সংক্ষিপ্ত বিবরণ {#overview}

`react-dom` package আপনাকে DOM সম্পর্কিত কিছু মেথড প্রদান করে যা আপনি আপনার এপের top-level এ ব্যবহার করতে পারেন। আপনি এর মাধ্যমে প্রয়োজন অনুযায়ী React মডেল থেকেও বেরিয়ে আসতে পারেন। আপনার অধিকাংশ component এরই এই মডিউল এর প্রয়োজন হবেনা।
...
React ইন্টারনেট এক্সপ্লোরার ৯ সহ সব জনপ্রিয় ব্রাউজারই সাপোর্ট করে, যদিও ইন্টারনেট এক্সপ্লোরার ৯ এবং ইন্টারনেট এক্সপ্লোরার ১০ এর জন্য [কিছু পলিফিল এর প্রয়োজন পড়ে](/docs/javascript-environment-requirements.html)।

> বিঃদ্রঃ
>
> আমরা পুরনো ব্রাউজার গুলো সাপোর্ট করিনা যাতে ES5 মেথড এর সাপোর্ট নেই, কিন্তু আপনি হয়তো দেখবেন আপনার এপ পুরনো ব্রাউজারেও ঠিকমতই চলছে যদি আপনি [es5-shim এবং es5-sham](https://github.com/es-shims/es5-shim) এর মত পলিফিল গুলো আপনার পৃষ্ঠায় সংযুক্ত করেন। কিন্তু এক্ষেত্রে আপনি সম্পূর্ণ আপনার উপর।
...
এই মেথড কোন React element কে DOM এর `container` এ মাউন্ট করে এবং কম্পোনেন্ট এর একটি [রেফারেন্স](/docs/more-about-refs.html) রিটার্ন করে(অথবা `null` ও রিটার্ন করতে পারে যদি [stateless কম্পোনেন্ট](/docs/components-and-props.html#functional-and-class-components) হয়)।
...
[`render()`](#render) এর মতই, কিন্তু একটি কন্টেইনার কে hydrate করার জন্য ব্যবহার করা হয় যার HTML [`ReactDOMServer`](/docs/react-dom-server.html) এর মাধ্যমে রেন্ডার করা হয়েছিল। React এক্ষেত্রে উপস্থিত markup এর সাথে event listener সংযুক্ত করার চেষ্টা করবে।

React আশা করে রেন্ডারকৃত content সার্ভার এবং ক্লায়েন্ট এর মধ্যে দেখতে অনুরূপ হবে। এটি text content এর ক্ষেত্রে কোন ভিন্নতা থাকলে তা সাময়িকভাবে সমাধান করতে পারে, কিন্তু আপনার এরকম ভিন্নতা কে বাগ হিসেবে গণ্য করে এগুলো সমাধান করা উচিত। ডেভেলপমেন্ট মোডে React এরকম ভিন্নতা গুলো সম্পর্কে hydration এর সময় আপনাকে সতর্ক করে। এরকম কোন গ্যারান্টি নেই যে attribute এর মধ্যে ভিন্নতা দেখা দিলে তা আপনা-আপনি সমাধান হয়ে যাবে। এটি আপনার এপের দক্ষতার জন্য অত্যন্ত গুরুত্বপূর্ণ কারণ অধিকাংশ এপে এমন ভিন্নতা খুবই বিরল এবং সব markup validate করা এপের দক্ষতা অনেকাংশে কমিয়ে দেয়।
...
> বিঃদ্রঃ
>
> `findDOMNode` নিম্নাবস্থিত DOM নোড এক্সেস করার একটি সর্বশেষ পথ। অধিকাংশ ক্ষেত্রে, এটি ব্যবহার করা থেকে নিরুৎসাহিত করা হয় কারণ এটি component abstraction কে ভেদ করে। [এটি `StrictMode` এ ব্যবহার করার অনুমোদন নেই।](/docs/strict-mode.html#warning-about-deprecated-finddomnode-usage)
...
এটি একটি পোর্টাল বা পথ তৈরি করে। এই পোর্টালগুলো [DOM কম্পোনেন্ট এর বাইরে থাকা কোন DOM নোডে children রেন্ডার](/docs/portals.html) করার একটা সমাধান প্রদান করে।
